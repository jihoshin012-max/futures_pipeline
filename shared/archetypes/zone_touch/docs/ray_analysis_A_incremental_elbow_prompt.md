...
| has_backing_ray | Binary: is there a 60m+ backing ray within 100t? |
...
| Strong backing | backing_ray_streak ≥ 5 AND backing_ray_dist ≤ 60t | ⚠️ CONDITIONAL on Step 1d: if paradox confirmed, this is a SKIP candidate (congestion), not protective |
...
For each skip gate candidate from 2c:
1. Apply the same ray context computation to P2 qualifying trades
