...
SCSFExport scsf_ZoneBounceSignalsV4(SCStudyInterfaceRef sc)
{
    ...
    for (int s = 0; s < activeCount; s++)
    {
        ...
        // v3.2 CHANGE: VP Ray proximity filter — stale subgraph values
        // filtered by 3x zone width threshold (2026-03-22)
        // vpActive is set per-touch below using zone-local context.
        // Raw vpRay kept for the proximity check.
                ...
                {
                    {
                        ...
                        sig.VPRayPrice       = vpNearZone ? vpRay : 0.0f;
                ...
                {
                    {
                        ...
                        // v3.2: VP proximity filter — supply touch price = sBot
                        bool vpNearZone = (vpRay > 0.0f) &&
