...
{
    SCString FilePath;
    FilePath.Format("%s\\ATEAM_ROTATION_V1_1_log.csv", sc.DataFilesFolder().GetChars());
...
SCSFExport scsf_ATEAM_ROTATION_V1_1(SCStudyInterfaceRef sc)
{
    if (sc.SetDefaults)
    {
        sc.GraphName = "ATEAM Rotation V1.1";
        sc.AutoLoop = 1;
        sc.UpdateAlways = 1;
        sc.GraphRegion = 0;
    ...
    // Full recalc reset — zero all persistent state to prevent stale values
    ...
    {
        SCString msg;
        msg.Format("Dir:%d Level:%d Pos:%d Anchor:%.2f Watch:%.2f Price:%.2f Pend:%d Flat:%d",
                   Direction, Level, PosQty, AnchorPrice, WatchPrice, Price, OrderPending, FlattenPending);
    ...
    // 1) Seed: wait for price to move StepDist from watch price before entering
    if (PosQty == 0 && AnchorPrice == 0.0)
    {
        ...
        // Phase B: wait for StepDist move to decide direction
        int SeedDir = 0;
        if (Price >= WatchPrice + StepDist)
            SeedDir = 1;   // long
        else if (Price <= WatchPrice - StepDist)
            SeedDir = -1;  // short
        else
