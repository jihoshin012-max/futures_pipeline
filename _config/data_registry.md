...
| zone_csv_v2     | touches | ZTE unified zone touch events (52 cols). Generated by ZoneTouchEngine v4.0 on 250-vol bars — replaces separate ZRA+ZB4 outputs. Includes measurement (reaction/penetration/threshold bars), scoring (A-Cal/mode/cascade), and ray data. Must be paired with bar_data_volume. File pattern kept as NQ_ZRA_Hist_*.csv for downstream compatibility (Decision #5, consolidation plan) | P1, P2  | NQ_ZRA_Hist_*.csv         | 03-hypothesis, 04-backtest |
| zone_csv_zb4    | touches | **DEPRECATED** — ZB4 signals now included in zone_csv_v2 (ZTE unified CSV). Retained for reference only | P1, P2  | NQ_ZB4_signals_*.csv      | — |
